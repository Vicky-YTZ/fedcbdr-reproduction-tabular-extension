...
class ReplayBuffer:
    ...
    def add_task_dataset(self, task_id, client_datasets, model=None, device="cpu", strategy="gdr"):
        ...
        # -------------------------------------------------------------
            ...
            for client_id in range(self.num_clients):
                feats = client_features[client_id]
                lbls = client_labels[client_id]
                selected_local_indices = self.select_kmeans_per_client(feats, lbls, samples_per_client)
                if len(selected_local_indices) > 0:
                    subset = Subset(candidate_subsets[client_id], selected_local_indices)
                    self.client_buffers[client_id].append(subset)
            ...
            for client_id in range(self.num_clients):
                feats = client_features[client_id]
                lbls = client_labels[client_id]
                selected_local_indices = self.select_svd_kmeans_per_client(feats, lbls, samples_per_client)
                if len(selected_local_indices) > 0:
                    subset = Subset(candidate_subsets[client_id], selected_local_indices)
                    self.client_buffers[client_id].append(subset)
    ...
    def select_herding_per_client(self, features, labels, samples_per_client):
        ...
        for cls in classes:
            ...
            selected_feat_sum = torch.zeros_like(class_mean) # initialize sum vector = 0
    ...
    def select_svd_kmeans_per_client(self, features, labels, samples_per_client):
        ...
        for cls in classes:
            ...
            class_features_compressed = torch.matmul(
                class_features - torch.mean(class_features, dim=0), 
                V
            ).cpu().numpy()
